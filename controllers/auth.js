...
export const smart_redirect = (req, res) => {
    ...
    console.log(`current logged in user is a  ${ROLE_CODE_DECODER[req.user.role]}, ${req.user.email}`);
    res.json({ redirect: url, data: req.user });
};

export const smart_redirect_google = (req, res) => {
    const url = (req.session.returnTo || "https://hudibia-frontend-test.onrender.com/dashboard");
    // res.redirect(url);
    res.json({ redirect: url, data: req.user });
};
